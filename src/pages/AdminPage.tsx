...
const api = {
    ...
    getUsers: async (params: Record<string, any>): Promise<PaginatedResponse<UserSummary>> => {
    ...
    updateCredits: async (userId: string, credits: number, reason: string, admin: any) => {
        const response = await fetch(`${VITE_BACKEND_URL}/admin/users/${userId}/credits`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${admin.token}`
    ...
    suspendUser: async (userId: string, reason: string, duration_days: number | null, admin: any) => {
        const response = await fetch(`${VITE_BACKEND_URL}/admin/users/${userId}/suspend`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${admin.token}`
    ...
    unsuspendUser: async (userId: string, admin: any) => {
        const response = await fetch(`${VITE_BACKEND_URL}/admin/users/${userId}/unsuspend`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${admin.token}`
    ...
    updateRole: async (userId: string, role: UserRole, reason: string, admin: any) => {
        const response = await fetch(`${VITE_BACKEND_URL}/admin/users/${userId}/role`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${admin.token}`
    ...
    updateNotes: async (userId: string, notes: string, admin: any) => {
        const response = await fetch(`${VITE_BACKEND_URL}/admin/users/${userId}/notes`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${admin.token}`
...
const EditCreditsModal: React.FC<EditCreditsModalProps> = ({ isOpen, onClose, user, onSuccess }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toaster.create({ title: 'Credits updated successfully', type: 'success' });
            onSuccess();
            onClose();
        } catch (error) {
    ...
    return (
        ...
        >
            ...
            >
                ...
                <Stack gap={4}>
                    <Box>
                        <Text fontSize="sm" mb={2}>User: {user.email}</Text>
                        <Text fontSize="xs" color="gray.600">Current credits: {user.credits}</Text>
                    </Box>
...
const SuspendUserModal: React.FC<SuspendUserModalProps> = ({ isOpen, onClose, user, onSuccess }) => {
    const [reason, setReason] = useState('');
    ...
    const handleSubmit = async () => {
        ...
        if (!reason.trim() || reason.length < 10) {
            toaster.create({ title: 'Please provide a detailed reason (min 10 characters)', type: 'error' });
            return;
        }
    ...
    return (
        ...
        >
            ...
            >
                <Flex justify="space-between" align="center" mb={4}>
                    <Heading size="lg" color="red.600">Suspend User</Heading>
                    <IconButton variant="ghost" size="sm" onClick={onClose}>
                        <X />
                    </IconButton>
                </Flex>
                ...
                <Stack gap={4}>
                    <Box>
                        <Text fontSize="sm" mb={2}>User: {user.email}</Text>
                        <Text fontSize="xs" color="red.600">This will prevent the user from accessing their account.</Text>
                    </Box>
                    ...
                    <Field.Root>
                        ...
                        <Field.HelperText>Leave empty for indefinite suspension</Field.HelperText>
                        <Input
                            type="number"
                            value={duration}
...
const UpdateRoleModal: React.FC<UpdateRoleModalProps> = ({ isOpen, onClose, user, onSuccess }) => {
        ...
        >
            ...
            >
                ...
                <Stack gap={4}>
                    <Box>
                        <Text fontSize="sm" mb={2}>User: {user.email}</Text>
                        <Text fontSize="xs" color="gray.600">Current role: {user.role}</Text>
                    </Box>
                    ...
                    {loading ? (
                        ...
                        <Tabs.Root value={activeTab} onValueChange={(e) => setActiveTab(e.value)}>
                            ...
                            <Tabs.Content value="overview" pt={4}>
                                <Stack gap={4}>
                                    <Grid templateColumns="repeat(2, 1fr)" gap={4}>
                                        ...
                                        <Box>
                                            <Text fontSize="sm" color="gray.600" mb={1}>Total Courses</Text>
                                            <Text fontWeight="medium">{user.total_courses}</Text>
                                        </Box>
                                        <Box>
                                            <Text fontSize="sm" color="gray.600" mb={1}>Completed Courses</Text>
                                            <Text fontWeight="medium">{user.completed_courses}</Text>
                                        </Box>
                                        <Box>
                                            <Text fontSize="sm" color="gray.600" mb={1}>Total Roadmaps</Text>
                                            <Text fontWeight="medium">{user.total_roadmaps}</Text>
                                        </Box>
                                        <Box>
                                            <Text fontSize="sm" color="gray.600" mb={1}>Login Count</Text>
                            ...
                            <Tabs.Content value="credits" pt={4}>
                                <Stack gap={4}>
                                    <Box>
                                        <Text fontSize="sm" color="gray.600" mb={1}>Current Credits</Text>
                                        <Heading size="xl">{user.credits}</Heading>
                                    </Box>
                                    <Box>
                                        <Text fontSize="sm" color="gray.600" mb={1}>Total Credits Used</Text>
                                        <Text fontSize="lg" fontWeight="medium">{user.total_credits_used}</Text>
                                    </Box>
                                    <Box>
                                        <Text fontSize="sm" color="gray.600" mb={1}>Credits Reset At</Text>
                                        <Text>{user.credits_reset_at ? new Date(user.credits_reset_at).toLocaleString() : 'N/A'}</Text>
                                    </Box>
                                </Stack>
                            </Tabs.Content>
                            ...
                            <Tabs.Content value="membership" pt={4}>
                                <Stack gap={4}>
                                    <Grid templateColumns="repeat(2, 1fr)" gap={4}>
                                        <Box>
                                            <Text fontSize="sm" color="gray.600" mb={1}>Plan</Text>
                                            <Badge colorPalette={user.membership_plan === 'premium' ? 'purple' : 'gray'} size="lg">
                                                {user.membership_plan}
                                            </Badge>
                                        </Box>
                                        <Box>
                                            <Text fontSize="sm" color="gray.600" mb={1}>Status</Text>
                                            <Badge colorPalette={user.membership_status === 'ACTIVE' ? 'green' : 'gray'} size="lg">
                                                {user.membership_status}
                                            </Badge>
                                        </Box>
                                        <Box>
                                            <Text fontSize="sm" color="gray.600" mb={1}>Active Until</Text>
                                            <Text>{user.membership_active_until ? new Date(user.membership_active_until).toLocaleDateString() : 'N/A'}</Text>
                                        </Box>
                                        <Box>
                                            <Text fontSize="sm" color="gray.600" mb={1}>Stripe Customer ID</Text>
                                            <Text fontSize="sm" fontFamily="mono">{user.stripe_customer_id || 'N/A'}</Text>
                                        </Box>
                                    </Grid>
                                </Stack>
                            </Tabs.Content>
                            ...
                            <Tabs.Content value="activity" pt={4}>
                                <Stack gap={4}>
                                    <Box>
                                        <Text fontSize="sm" color="gray.600" mb={1}>Last Login</Text>
                                        <Text>{user.last_login_at ? new Date(user.last_login_at).toLocaleString() : 'Never'}</Text>
                                    </Box>
                                    <Box>
...
const AdminDashboard: React.FC = () => {
    ...
    const handleUnsuspendUser = async (userId: string) => {
        try {
            ...
            toaster.create({ title: 'User unsuspended successfully', type: 'success' });
            loadData();
        } catch (error) {
            ...
            <Box borderBottom="1px" borderColor="gray.200" px={8} py={4}>
                <Container maxW="7xl">
                    <Flex justify="space-between" align="center">
                        ...
                        <Badge colorPalette="purple" size="lg">{admin?.role || 'Admin'}</Badge>
                    </Flex>
                </Container>
            </Box>
            ...
            <Container maxW="7xl" py={8}>
                <Stack gap={8}>
                    ...
                    {stats && (
                        <Card.Root>
                            <Card.Body>
                                <Heading size="md" mb={4}>New Users</Heading>
                                ...
                                <Table.Body>
                                    {users.map((user) => (
                                        <Table.Row key={user.id}>
                                            <Table.Cell fontWeight="medium">{user.name || 'N/A'}</Table.Cell>
                                            ...
                                            <Table.Cell>
                                                <Badge
                                                    colorPalette={user.membership_plan === 'premium' ? 'purple' : 'gray'}
                                                    size="sm"
                                                >
                                                    {user.membership_plan}
                                                </Badge>
