...
function ProductsStep({
  ...
  onSelectPlan: (plan: (typeof products)[0]) => void;
}) {
  const [loading, setLoading] = useState(false);
  const [cancellationResponse, setCancellationResponse] = useState<any | null>(
    null,
  );
  const [showCancelDialog, setShowCancelDialog] = useState(false);
  const { user } = useUser();
  const showCancelButton =
    user?.membership_plan === "premium" && user?.membership_status === "ACTIVE";
...
function PaymentStep({
  ...
  return (
    <VStack gap={6} align="stretch" maxW="2xl" mx="auto">
      <Button variant="ghost" onClick={onBack} size="sm" alignSelf="flex-start">
        <ArrowLeft size={16} />
        Back to Plans
      </Button>
...
export default function PaymentStepper() {
  ...
  const [selectedPlan, setSelectedPlan] = useState<(typeof products)[0] | null>(
    null,
  );
